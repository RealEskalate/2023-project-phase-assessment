...
import { useGetDoctorsQuery, useSearchDoctorsMutation } from "@/store/features/doctors/doctorsApi";
...
const DoctorsPage = () => {
  ...
  const [searchDoctors, {data: searchResults, isError: searchError, isLoading: searchLoading}] = useSearchDoctorsMutation()
  
  // Create a function to handle search input changes and trigger the search query
  ...
  return (
    <div>
      ...
      <div className="mt-10 w-3/4 mx-auto grid grid-cols-4 gap-x-4 gap-y-5">
        {searchTerm !== '' && searchResults?.data?.map((doctor: Doctor) => (
          <SingleDoctor
            key={doctor._id}
            _id={doctor._id}
        ...
        {searchTerm === '' && data?.data?.map((doctor: Doctor) => (
          <SingleDoctor
            key={doctor._id}
            _id={doctor._id}
            availability={doctor.availability}
